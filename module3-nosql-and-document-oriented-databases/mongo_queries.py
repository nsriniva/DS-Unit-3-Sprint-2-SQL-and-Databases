...
CHARACTER_ITEMS_LIST = 'SELECT character_id, group_concat(armory_item.name)  as items FROM charactercreator_character_inventory JOIN armory_item on charactercreator_character_inventory.item_id = armory_item.item_id GROUP BY character_id'

CHARACTER_WEAPONS_LIST = 'SELECT character_id, group_concat(armory_item.name)  as weapons FROM charactercreator_character_inventory JOIN armory_item on charactercreator_character_inventory.item_id = armory_item.item_id WHERE charactercreator_character_inventory.item_id in (select item_ptr_id from armory_weapon) GROUP BY character_id'
...
for rpg_table in rpg_tables:
    ...
    #print(rpg_tables[rpg_table].to_json(orient='records'))
